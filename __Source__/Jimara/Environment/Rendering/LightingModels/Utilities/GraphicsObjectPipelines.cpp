			...
			inline Reference<const BindingSetInstance> Get(
				...
				return GetCachedOrCreate(viewportData, [&]() -> Reference<BindingSetInstance> {
					...
					// Create vertex input:
					{
						...
						constBindings.clear();
						result->vertexInput = pipeline->CreateVertexInput(constBindings.data(), result->vertexBuffer->IndexBuffer());
						constBindings.clear();
...
#pragma endregion
	...
	void GraphicsObjectPipelines::ObjectInfo::ExecutePipeline(const Graphics::InFlightBufferInfo& inFlightBuffer)const {
		...
		const size_t instanceCount = (geometry == nullptr) ? size_t(0u) : (size_t)geometry->DrawInstanceCount();
		if (instanceCount <= 0u) 
			return;

		// Check index count:
		const size_t indexCount = (geometry == nullptr) ? size_t(0u) : (size_t)geometry->IndexCount();
		if (indexCount <= 0u)
			return;
