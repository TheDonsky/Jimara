			...
			public:
				...
				inline virtual Graphics::BindingSet::BindingSearchFunctions BindingSearchFunctions()const override {
					...
					static Reference<const Graphics::ResourceBinding<Graphics::TextureSampler>>
						...
						const size_t texturePropId = self->Shader()->PropertyIdByName(FontTextureShaderBindingName());
						if (texturePropId != Material::NO_ID && self->Shader()->Property(texturePropId).bindingName == desc.name)
							return self->m_atlas.textureBinding;
				...
				inline virtual void GetGeometry(GraphicsObjectDescriptor::GeometryDescriptor& descriptor)const override {
					// Vertex fields:
					{
						const Reference<Graphics::ArrayBuffer> vertexBuffer = m_textMesh.vertices->BoundObject();
					...
					// Instance fields:
					{
						const Reference<Graphics::ArrayBuffer> instanceBuffer = m_instanceData.binding->BoundObject();
						{
