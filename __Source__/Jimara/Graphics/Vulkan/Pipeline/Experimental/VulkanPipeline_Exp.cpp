			...
			struct VulkanPipeline::Helpers {
				...
				inline static bool CreateDescriptorSetLayout(VulkanDevice* device, size_t setIndex, DescriptorSetInfo& setInfo) {
					...
					auto addBinding = [](PipelineStageMask stages, size_t shaderBinding, VkDescriptorType type) {
						...
						binding.stageFlags =
							(((stages & StageMask(PipelineStage::COMPUTE)) != 0) ? VK_SHADER_STAGE_COMPUTE_BIT : 0) |
							(((stages & StageMask(PipelineStage::VERTEX)) != 0) ? VK_SHADER_STAGE_VERTEX_BIT : 0) |
							(((stages & StageMask(PipelineStage::FRAGMENT)) != 0) ? VK_SHADER_STAGE_FRAGMENT_BIT : 0);
