...
namespace Jimara {
	namespace OS {
		...
		namespace Clipboard {
			...
			JIMARA_API bool Clear(Logger* logger = nullptr);
			...
			/// <param name="logger"> Logger for error reporting (optional) </param>
			/// <returns> True, if successful </returns>
			JIMARA_API bool SetText(std::string_view text, Logger* logger = nullptr);
			...
			JIMARA_API std::optional<std::string> GetText(Logger* logger = nullptr);
			...
			/// <param name="logger"> Logger for error reporting (optional) </param>
			/// <returns> True, if successful </returns>
			JIMARA_API bool SetData(std::string_view typeId, MemoryBlock data, Logger* logger = nullptr);
			...
			JIMARA_API MemoryBlock GetData(std::string_view typeId, Logger* logger = nullptr);
		}
	}
}
